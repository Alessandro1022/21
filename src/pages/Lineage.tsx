      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span
              className={`text-sm font-serif truncate transition-colors duration-200 ${
                isSelected
                  ? "text-primary"
                  : "text-[#111111] dark:text-foreground drop-shadow-sm"
              }`}
            >
              {l.name}
            </span>
          ...
          <div className="flex items-center gap-2 text-[10px] font-sans text-[#333333] dark:text-muted-foreground">
            <span>
              {formatYear(l.reignStart, language)}–{formatYear(l.reignEnd, language)}
            </span>
        ...
        <div className="hidden sm:flex items-center gap-2 flex-shrink-0">
          <div className="w-16 h-1.5 rounded-full bg-secondary overflow-hidden">
            <div
              ...
              style={{
                width: `${Math.min(100, (reignLength / 50) * 100)}%`,
                backgroundColor: dynastyColor,
          ...
          <div className="bg-card/90 backdrop-blur-md rounded-xl ottoman-border p-5 max-w-md shadow-xl">
            ...
            <div className="space-y-2 mb-3">
              <div className="flex items-center gap-2 text-xs font-sans text-muted-foreground">
                ...
                {formatYear(l.reignStart, language)}–{formatYear(l.reignEnd, language)} (
                {reignLength}{" "}
                {language === "sv" ? "år" : language === "tr" ? "yıl" : "years"})
              </div>
              <p className="text-xs font-sans text-[#222222] dark:text-foreground/80">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-sans text-muted-foreground">
                  {getDynastyLabel(l.generation, empireId, language)}
                </span>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="h-2 bg-secondary rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.min(100, (reignLength / 50) * 100)}%` }}
                />
              </div>
            </div>
...
export default function Lineage() {
  ...
  const leaders: any[] = config?.leaders || [];
  const profiles: any[] = config?.profiles || [];
 
  const labels = {
    sv: { title: "Kejserlig stamtavla", count: "härskare" },
    en: { title: "Imperial Lineage", count: "rulers" },
    tr: { title: "İmparatorluk Soyu", count: "hükümdar" },
  };
            ...
            {[
              ...
              <div key={item.gen} className="flex items-center gap-1.5">
                <div
                  className="w-2 h-2 rounded-full"
                  style={{
                    backgroundColor: getDynastyColor(item.gen, empireId || "ottoman"),
                  }}
                />
                <span className="text-[10px] font-sans text-muted-foreground">
                  {getDynastyLabel(item.gen, empireId || "ottoman", language)}
                </span>
              </div>
            ))}
          </div>
 
          {/* Flat list — no tree, no indentation */}
          <div className="space-y-0.5">
            {leaders.map((leader) => (
              <LeaderRow
                ...
                onOpenModal={(l) => setModalLeader(l)}
              />
            ))}
          </div>
        </div>
