      ...
      style={
        bgImage
          ? {
              ...
              paddingTop: "env(safe-area-inset-top)",   // ← pushar HELA sidan ner
            }
          : { paddingTop: "env(safe-area-inset-top)" }
      }
    >
      {/* Mörkare overlay – /95 ger bättre läsbarhet */}
      <div className="absolute inset-0 bg-background/95" />

      <header
        className="relative z-10 flex items-center justify-between px-6"
        style={{
          paddingTop: "max(env(safe-area-inset-top), 16px)", // behåll din extra padding
          paddingBottom: "16px",
        }}
      >
          ...
          {user ? (
            <>
              {isAdmin && (
                <Link
                  to="/admin"
                  className="px-3 py-1.5 rounded-lg bg-accent/20 text-accent font-sans text-xs sm:text-sm flex items-center gap-1 hover:bg-accent/30 transition-colors"
                >
              ...
              <button
                onClick={signOut}
                className="px-3 py-1.5 rounded-lg bg-secondary text-secondary-foreground font-sans text-xs sm:text-sm hover:bg-muted transition-colors"
              >
                {logoutLabel}
              </button>
            </>
            ...
            <Link
              to="/auth"
              className="px-3 py-1.5 rounded-lg gold-gradient text-primary-foreground font-sans text-xs sm:text-sm flex items-center gap-1"
            >
      ...
      {/* resten av din kod är oförändrad – main, features, modules, footer */}
        ...
        <div className="flex flex-col items-center text-center pt-10 pb-6 max-w-2xl animate-fade-in">
          ...
          {user ? (
            <div className="flex flex-wrap gap-3 justify-center">
              <Link
                to="/chat"
                className="px-6 py-2.5 rounded-xl gold-gradient text-primary-foreground font-sans text-sm sm:text-base font-medium flex items-center gap-1.5 hover:opacity-90 transition-opacity ottoman-glow"
              >
                <MessageSquare className="w-4 h-4 sm:w-5 sm:h-5" /> {startLabel}{" "}
                <ChevronRight className="w-3 h-3 sm:w-4 sm:h-4" />
              </Link>
              <Link
                to="/select-empire"
                className="px-5 py-2.5 rounded-xl bg-secondary text-secondary-foreground font-sans text-sm sm:text-base flex items-center gap-1.5 hover:bg-muted transition-colors ottoman-border"
              >
            ...
            <Link
              to="/auth"
              className="px-6 py-3 rounded-xl gold-gradient text-primary-foreground font-sans text-sm sm:text-base font-medium flex items-center gap-1.5 hover:opacity-90 transition-opacity ottoman-glow"
            >
        ...
        <div className="max-w-4xl w-full pb-6">
          ...
          <div className="grid sm:grid-cols-3 gap-3 mb-8">
            {FEATURES.map((f, i) => (
              <div
                key={i}
                className="bg-card/60 backdrop-blur-sm rounded-2xl ottoman-border p-4 text-center animate-fade-in"
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
              {MODULES.map((m, i) => (
                ...
                >
                  ...
                  <span className="text-xs sm:text-sm font-serif text-foreground mb-0.5">
                    {m.label[language as keyof typeof m.label] || m.label.en}
                  </span>
                  <span className="text-[9px] sm:text-xs font-sans text-muted-foreground">
