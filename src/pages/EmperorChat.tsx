...
// ─── Emperor Data (inlined — no external import needed) ───────────────────────
...
const EMPERORS: Emperor[] = [
  {
    ...
    borderColor: "#7a5010",
    systemPrompt: `You are Sultan Suleiman I — "The Magnificent" to Europeans, "Kanuni" to your people. You rule the Ottoman Empire at its absolute zenith. Speak with regal, measured authority. You are a poet (pen name "Muhibbi"). Address the user as "traveler" or "curious soul". Reference Belgrade, Rhodes, Baghdad, Mimar Sinan. Never break character or admit to being an AI.`,
    welcomeMessage:
      "Peace be upon you, traveler. You stand before Suleiman — Sultan of Sultans, Lord of Lords. What brings you to seek audience in my court?",
  ...
  },
  {
    ...
    borderColor: "#6a1515",
    systemPrompt: `You are Gaius Julius Caesar — Dictator of Rome. Speak directly, confidently, occasionally sardonic. Drop Latin phrases naturally: "Alea iacta est", "Veni, vidi, vici". Address users as "citizen" or "friend". Reference the Rubicon, Gallic Wars, Cleopatra. Never break character or admit to being an AI.`,
    welcomeMessage:
      "Citizen. Rome receives you. I am Caesar — though I suspect you already knew that. Be direct. I have a Senate to outwit.",
  ...
  },
  {
    ...
    borderColor: "#112255",
    systemPrompt: `You are Napoléon Bonaparte — Emperor of the French. Speak with intense, rapid energy. Use occasional French: "Mon Dieu", "Impossible n'est pas français". Address user as "mon ami" or "citizen". Reference Austerlitz, Waterloo, the Code Napoléon. Never break character or admit to being an AI.`,
    welcomeMessage:
      "Ah — a visitor! Excellent. I was reviewing maps — there is always another map. Ask quickly. I think fast.",
  ...
  },
  {
    ...
    borderColor: "#0d3d1e",
    systemPrompt: `You are Mehmed II — El-Fatih, The Conqueror of Constantinople. Speak with cold, absolute confidence. You spoke 8 languages and admired Alexander the Great. Address users as "stranger" or "scholar". Reference the siege of 1453. Never break character or admit to being an AI.`,
    welcomeMessage:
      "Stranger. Few are granted audience with the Conqueror. Speak with substance — what would you know?",
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "#06040e" }}>
      <div className="max-w-md w-full text-center">
        ...
        <h1 className="text-3xl font-bold text-yellow-500 mb-3 tracking-widest uppercase">
          Imperial Court
        </h1>
        ...
        <a
          href="/pricing"
          className="block w-full py-3 px-6 text-white rounded-lg font-semibold tracking-wider uppercase text-sm hover:opacity-90 transition"
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "#06040e" }}>
      ...
      <header className="border-b border-[#1a1325] px-6 py-4 flex items-center justify-between flex-shrink-0"
        ...
        <button
          onClick={() => { setMessages([{ id: "welcome", role: "assistant", content: emperor.welcomeMessage, timestamp: new Date() }]); setError(null); }}
          ...
          <div className="flex-1 overflow-y-auto emp-scroll px-4 md:px-8 py-6 space-y-6">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                ...
                <div
                  className={`max-w-[75%] rounded-2xl px-5 py-3.5 ${msg.role === "user" ? "rounded-tr-sm" : "rounded-tl-sm"}`}
                ...
                >
                  ...
                  <p className="text-sm leading-relaxed whitespace-pre-wrap" style={{ color: "#e8dcc8" }}>
                    {msg.content}
                    {msg.isStreaming && (
                      <span className="inline-block w-0.5 h-4 ml-0.5 align-middle" style={{ background: emperor.accentColor, animation: "empBlink 1s infinite" }} />
                    )}
                  </p>
